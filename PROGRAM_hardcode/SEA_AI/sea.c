...
void SeaLogin(ref Login)
{
	...
	// login island if exist
		if (sIslandID != "")
		{
			...
			//Island.FogDensity = stf(Weather.Fog.density) * 4;
			...
			float islandFogDensity = stf(Weather.Fog.density) * 0.75;
            Island.FogDensity = islandFogDensity;
            SendMessage(&IslandReflModel, "lllf", 20511, 1, 1, stf(Weather.Fog.density) * islandFogDensity);
