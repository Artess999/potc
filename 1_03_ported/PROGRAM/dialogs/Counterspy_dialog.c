//nclude "DIALOGS\Counterspy_dialog.h"
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "After_Fight_2":
			...
			d.Text = DLG_TEXT[30] + Characters[getCharacterIndex(DLG_TEXT[31])].name + " " + Characters[getCharacterIndex(DLG_TEXT[32])].lastname + DLG_TEXT[33];
			Link.l1 = DLG_TEXT[34];
		...
		case "Officer_money_4":
			...
			d.Text =  DLG_TEXT[47] + Characters[GetCharacterIndex(DLG_TEXT[48])].name + DLG_TEXT[49];
			link.l1 = DLG_TEXT[50];
		...
		case  "Officer_money_5":
			...
			d.Text = Characters[GetCharacterIndex(DLG_TEXT[51])].name + " " + Characters[GetCharacterIndex(DLG_TEXT[52])].lastname + DLG_TEXT[53];
			Link.l1 = DLG_TEXT[54];
			Link.l1.go = "exit";
		...
		case "Talk_AfterTavern":
			...
			d.Text = DLG_TEXT[61] + Characters[GetCharacterIndex(DLG_TEXT[62])].name + DLG_TEXT[63];
			Link.l1 = DLG_TEXT[64] + Characters[GetCharacterIndex(DLG_TEXT[65])].lastname + DLG_TEXT[66];
			Link.l1.go = "Talk_AfterTavern_1";
		break;

		case "Talk_AfterTavern_1":
			dialog.snd = "Voice\COUN\COUN028";
			d.Text = DLG_TEXT[67] + Characters[GetCharacterIndex(DLG_TEXT[68])].name + DLG_TEXT[69];
			Link.l1 = DLG_TEXT[70];
